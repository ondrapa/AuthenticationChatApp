...
import com.example.chineseappfinal.data.model.AuthState
    ...
    fun onEvent(event: AuthUiEvent) {
        when(event) {
            ...
            is AuthUiEvent.SignUpFullNameChanged -> {
                state = state.copy(signUpFullName = event.value)
            }
    ...
    private fun signUp() {
        viewModelScope.launch {
            ...
            val result = repository.signUp(
                name = state.signUpFullName.split(" ")[0],
                surname = state.signUpFullName.split(" ")[1],
